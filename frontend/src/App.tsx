...
import { Stethoscope, ClipboardList, Loader2, AlertCircle, CheckCircle, BrainCircuit, Sparkles, UserRoundSearch } from 'lucide-react';
      ...
      <main className="max-w-6xl mx-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="bg-white p-6 rounded-xl shadow-sm border">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <textarea
              className="w-full h-64 p-4 border rounded-xl bg-slate-50 outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            ...
            <button
              disabled={loading}
              className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold hover:bg-blue-700 disabled:bg-slate-300 flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        <section className="space-y-4">
          ...
          {!result && !loading && !error && (
            <div className="h-full min-h-[400px] border-2 border-dashed rounded-xl flex flex-col items-center justify-center text-slate-400">
          ...
          {result && (
            <div className="space-y-4 animate-in fade-in duration-500">
              <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                ...
                <p className="text-sm italic text-slate-700">"{result.resumo_clinico}"</p>
              </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-emerald-100 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {result.diagnosticos_identificados.map((d, i) => (
                      <span key={i} className="text-[10px] bg-emerald-50 text-emerald-700 px-2 py-1 rounded font-bold">{d}</span>
                    ))}
                  </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {result.sugestoes_ia.map((s, i) => (
                      <span key={i} className="text-[10px] bg-blue-50 text-blue-700 px-2 py-1 rounded font-bold">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
